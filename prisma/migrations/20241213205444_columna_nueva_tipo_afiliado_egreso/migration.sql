/*
  ...
  - Added the required column `fk_tipo_afiliado` to the `egreso` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `egreso` ADD COLUMN `fk_tipo_afiliado` INTEGER NOT NULL;
...
-- AddForeignKey
ALTER TABLE `egreso` ADD CONSTRAINT `egreso_fk_tipo_afiliado_fkey` FOREIGN KEY (`fk_tipo_afiliado`) REFERENCES `afiliados`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
